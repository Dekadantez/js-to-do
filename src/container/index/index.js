export class Todo {
  ...
  static #createTaskData = (text) => {
    //додає новий обєкт нашої задачи до #list
    this.#list.push({
      id: ++this.#count,
      text,
      done: false,
  ...
  static #handleAdd = () => {
    //метод який отримує значення та передає значення на створення задачи
    const value = this.#input.value

    if (value.length > 1) {
      this.#createTaskData(value)
      this.#input.value = '' //щоб після створення нової задачи поле інпута очищувалося
      this.#render()
      this.#saveData()
    }
  }
  ...
  static #render = () => {
    //метод який відображає список задач на сторінці
  ...
  //   метод який буде створювати елемент нашої задачи з привязкою до кнопок
  ...
  static #handleCancel = (data) => () => {
    //функція для видалення нашої задачи
  ...
  static #deleteById = (id) => {
    //функція для видалення нашої задачи по id після видалення завдання
